...
import { container, pageTitle, pageSubtitle, searchField, btnPrimary } from "../components/ui";
...
import { Loader2, Trash2, X, AlertCircle, FileText, Calendar, Search, CalendarDays, ArrowUpDown, BarChart3, Grid3x3, List } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const [typePreview, setTypePreview] = useState<ContentType>(getFirstAvailableContentType());
  ...
  const filteredTemplates = useMemo(() => {
    ...
    if (templateStatusFilter.length > 0) {
      filtered = filtered.filter((tpl) => templateStatusFilter.includes(tpl.status));
    }
    
    // Apply search filter
    if (searchTerm.trim()) {
      const term = searchTerm.toLowerCase();
      filtered = filtered.filter((tpl) => tpl.template_name.toLowerCase().includes(term));
    }
    ...
    const sorted = [...filtered].sort((a, b) => {
      switch (sortBy) {
        ...
        case "date-newest":
          return new Date(b.log.updated_at || b.log.created_at).getTime() - 
                 new Date(a.log.updated_at || a.log.created_at).getTime();
        case "date-oldest":
          return new Date(a.log.updated_at || a.log.created_at).getTime() - 
  ...
  const filteredCards = useMemo(() => {
    ...
    if (cardStatusFilter.length > 0) {
      filtered = filtered.filter((card) => cardStatusFilter.includes(card.status));
    }
    
    // Apply search filter
    if (searchTerm.trim()) {
      const term = searchTerm.toLowerCase();
      filtered = filtered.filter((card) => card.card_name.toLowerCase().includes(term));
    }
  ...
  const filteredBulletins = useMemo(() => {
    ...
    if (bulletinStatusFilter.length > 0) {
      filtered = filtered.filter((bull) => bulletinStatusFilter.includes(bull.status));
    }
    
    // Apply search filter
    if (searchTerm.trim()) {
      const term = searchTerm.toLowerCase();
      filtered = filtered.filter((bull) => bull.bulletin_name.toLowerCase().includes(term));
    }
  ...
  const toggleSelectAll = () => {
    const currentItems = active === 'bulletins' ? filteredBulletins :
                         active === 'templates' ? filteredTemplates :
                         filteredCards;
              ...
              <div className='m-3 flex flex-wrap gap-3 justify-center'>
                <div className='flex items-center gap-2 px-4 py-2 bg-white rounded-lg border border-gray-200 shadow-sm'>
                  ...
                  <div>
                    ...
                    <p className='text-lg font-bold text-[#283618]'>{currentStats.total}</p>
                  </div>
                </div>
                <div className='flex items-center gap-2 px-4 py-2 bg-white rounded-lg border border-gray-200 shadow-sm'>
                  ...
                  <div>
                    ...
                    <p className='text-lg font-bold text-[#283618]'>{currentStats.thisWeek}</p>
                  </div>
                </div>
                {active === 'bulletins' && currentStats.pending > 0 && (
                  <div className='flex items-center gap-2 px-4 py-2 bg-white rounded-lg border border-orange-200 shadow-sm'>
                    ...
                    <div>
                      <p className='text-xs text-gray-500'>{t('stats.pendingReview')}</p>
                      <p className='text-lg font-bold text-orange-600'>{currentStats.pending}</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex gap-1 bg-gray-100 p-1 rounded-lg">
                  ...
                  >
                    ...
                    <span className="hidden sm:inline text-sm">{t('viewMode.grid')}</span>
                  </button>
                  <button
                  ...
                  >
                    ...
                    <span className="hidden sm:inline text-sm">{t('viewMode.table')}</span>
                  </button>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                {viewMode === 'grid' && (
                  <div className="flex items-center gap-2 w-full md:w-auto md:order-last md:ml-auto">
                    ...
                    <label className="text-sm text-gray-600 font-medium whitespace-nowrap">{t('sortBy.label')}:</label>
                    <select
                      value={sortBy}
                    ...
                    >
                      <option value="date-newest">{t('sortBy.date-newest')}</option>
                      <option value="date-oldest">{t('sortBy.date-oldest')}</option>
                      <option value="name-asc">{t('sortBy.name-asc')}</option>
                      <option value="name-desc">{t('sortBy.name-desc')}</option>
                    </select>
                  </div>
                )}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <label className="text-sm text-gray-600 font-medium whitespace-nowrap">{t('dateFilter.from')}:</label>
                  <input
                    type="date"
                    value={dateFrom}
                ...
                {/* Date To */}
                <div className="flex items-center gap-2">
                  <label className="text-sm text-gray-600 font-medium whitespace-nowrap">{t('dateFilter.to')}:</label>
                  <input
                    type="date"
                    value={dateTo}
              ...
              {active === 'bulletins' && (
                <>
                  ...
                  {bulletinStatuses.map((status) => (
                    ...
                    >
                      {t(`statusFilter.${status}`)} ({bulletinStatusCounts[status] || 0})
                    </button>
                  ))}
                </>
              )}
              ...
              {active === 'templates' && (
                <>
                  ...
                  {templateStatuses.map((status) => (
                    ...
                    >
                      {t(`statusFilter.${status}`)} ({templateStatusCounts[status] || 0})
                    </button>
                  ))}
                </>
              )}
              ...
              {active === 'cards' && (
                <>
                  ...
                  {cardStatuses.map((status) => (
                    ...
                    >
                      {t(`statusFilter.${status}`)} ({cardStatusCounts[status] || 0})
                    </button>
                  ))}
                </>
              )}
          ...
          <>
            {filteredTemplates.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 text-lg">{searchTerm ? t('noResults') : t('noTemplates', { default: 'No hay plantillas disponibles' })}</p>
              </div>
            ) : viewMode === 'grid' ? (
                  ...
                  .map((template) => {
                    ...
                    return (
                      <ItemCard
                        ...
                        editBtn={canEdit}
                        onEdit={canEdit ? () => router.push(`/templates/${template._id}/edit`) : undefined}
                        deleteBtn={canDelete}
              ...
              <TableView
                ...
                onDelete={(item) => handleDelete(item._id!, 'templates', item.template_name)}
                canEdit={(item) => can(PERMISSION_ACTIONS.Update, MODULES.TEMPLATE_MANAGEMENT, item.access_config?.allowed_groups || [])}
                canDelete={(item) => can(PERMISSION_ACTIONS.Delete, MODULES.TEMPLATE_MANAGEMENT, item.access_config?.allowed_groups || [])}
                getItemId={(item) => item._id!}
                getItemName={(item) => item.template_name}
                getItemStatus={(item) => item.status}
                getItemAuthor={(item) => `${item.log.updater_first_name || item.log.creator_first_name || ''} ${item.log.updater_last_name || item.log.creator_last_name || ''}`}
                getItemCreatedDate={(item) => item.log.created_at ? new Date(item.log.created_at).toLocaleDateString() : ''}
                getItemUpdatedDate={(item) => item.log.updated_at ? new Date(item.log.updated_at).toLocaleDateString() : ''}
        ...
        {!loading && !error && active === 'bulletins' && (
          <>
            {filteredBulletins.length === 0 ? (
              <div className="text-center py-12">
                <FileText className="h-16 w-16 text-gray-300 mx-auto mb-4" />
                <p className="text-gray-500 text-lg">{searchTerm ? t('noResults') : t('noBulletins', { default: 'No hay boletines disponibles' })}</p>
              </div>
            ) : viewMode === 'grid' ? (
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
                {filteredBulletins.map((b) => {
                    const allowedGroups = b.access_config?.allowed_groups || [];
                    const canEdit = can(PERMISSION_ACTIONS.Update, MODULES.BULLETINS_COMPOSER, allowedGroups);
                    const canDelete = can(PERMISSION_ACTIONS.Delete, MODULES.BULLETINS_COMPOSER, allowedGroups);
                    const creatorName =
                      b.log.creator_first_name && b.log.creator_last_name
                        ? `${b.log.creator_first_name} ${b.log.creator_last_name}`
                        : b.log.creator_first_name || b.log.creator_last_name || b.log.creator_user_id;
                    ...
                    return (
                      <ItemCard
                        ...
                        editBtn={canEdit}
                        onEdit={canEdit ? () => router.push(`/bulletins/${b._id}/edit`) : undefined}
                        deleteBtn={canDelete}
                        onDelete={canDelete ? () => handleDelete(b._id!, 'bulletins', b.bulletin_name) : undefined}
              ...
              <TableView
                ...
                onDelete={(item) => handleDelete(item._id!, 'bulletins', item.bulletin_name)}
                canEdit={(item) => can(PERMISSION_ACTIONS.Update, MODULES.BULLETINS_COMPOSER, item.access_config?.allowed_groups || [])}
                canDelete={(item) => can(PERMISSION_ACTIONS.Delete, MODULES.BULLETINS_COMPOSER, item.access_config?.allowed_groups || [])}
                getItemId={(item) => item._id!}
                ...
                getItemAuthor={(item) => {
                  ...
                  return firstName && lastName ? `${firstName} ${lastName}` : firstName || lastName || userId;
                }}
                getItemCreatedDate={(item) => item.log.created_at ? new Date(item.log.created_at).toLocaleDateString() : ''}
                getItemUpdatedDate={(item) => item.log.updated_at ? new Date(item.log.updated_at).toLocaleDateString() : ''}
                translationNamespace="Dashboard"
        ...
        {!loading && !error && active === 'cards' && (
          <>
            {filteredCards.length === 0 ? (
              <div className="text-center py-12">
                <FileText className="h-16 w-16 text-gray-300 mx-auto mb-4" />
                <p className="text-gray-500 text-lg">{searchTerm ? t('noResults') : t('noCards', { default: 'No hay tarjetas disponibles' })}</p>
              </div>
            ) : viewMode === 'grid' ? (
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
                {filteredCards.map((c) => {
                  ...
                  return (
                    <ItemCard
                      ...
                      editBtn={canEdit}
                      onEdit={canEdit ? () => router.push(`/cards/${c._id}/edit`) : undefined}
                      deleteBtn={canDelete}
                      onDelete={canDelete ? () => handleDelete(c._id!, 'cards', c.card_name) : undefined}
              ...
              <TableView
                ...
                onDelete={(item) => handleDelete(item._id!, 'cards', item.card_name)}
                canEdit={(item) => can(PERMISSION_ACTIONS.Update, MODULES.CARD_MANAGEMENT, item.access_config?.allowed_groups || [])}
                canDelete={(item) => can(PERMISSION_ACTIONS.Delete, MODULES.CARD_MANAGEMENT, item.access_config?.allowed_groups || [])}
                getItemId={(item) => item._id!}
