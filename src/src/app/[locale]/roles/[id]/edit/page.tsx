...
export default function EditRolePage() {
  ...
  // Cargar datos del rol al montar
  useEffect(() => {
    loadRoleData();
  }, [roleId]);
  ...
  const loadRoleData = async () => {
    ...
    try {
      ...
      showToast("Error de conexión al cargar el rol", "error", 4000);
      router.push("/roles");
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await RoleAPIService.updateRole(roleId, {
          ...
          <section className="bg-white py-10">
            <div className={container}>
              <div className="flex justify-center items-center py-20">
                ...
                <span className="ml-3 text-[#283618]/70">
                  {t("loadingRole")}
                </span>
              </div>
            </div>
