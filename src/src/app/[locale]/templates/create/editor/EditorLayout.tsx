...
export const EditorLayout: React.FC<EditorLayoutProps> = ({
  ...
  useEffect(() => {
      const handleKeyDown = (e: KeyboardEvent) => {
          if ((e.key === 'Delete' || e.key === 'Backspace') && selection.id && selection.type !== 'template') {
  ...
  const handleAddSection = () => {
      onUpdate(prev => {
          const newData = { ...prev };
          newData.version.content.sections.push({
              section_id: crypto.randomUUID(),
              display_name: `Section ${newData.version.content.sections.length + 1}`,
              order: newData.version.content.sections.length,
              blocks: [],
              background_url: [],
              icon_url: ''
          });
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-80px)] w-full bg-gray-100 overflow-hidden">
      <TopBar 
        ...
        onNameChange={(name) => onUpdate(prev => ({ ...prev, master: { ...prev.master, template_name: name } }))}
        saving={saving}
        lastSaved={lastSaved}
        onBack={onBack}
        onSave={onSave}
