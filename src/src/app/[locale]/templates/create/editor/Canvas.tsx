...
export const Canvas: React.FC<CanvasProps> = ({
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    const target = e.target as HTMLElement;
    const isInteractive = target.closest('button') || target.closest('input') || target.closest('.interactive-element');
  ...
  const handleElementClick = (type: "section" | "block" | "field" | "header" | "footer" | "header_field" | "footer_field", id: string, e: React.MouseEvent) => {
    e.stopPropagation();
            ...
            if (block.fields) {
                for(let fIdx = 0; fIdx < block.fields.length; fIdx++) {
                    ...
                    if (field.field_id === id || (field as any)._id === id) {
                         onSelect({ type: 'field', id, sectionIndex: sIdx, blockIndex: bIdx, fieldIndex: fIdx });
                         return;
                    }
                }
            }
    ...
    >
      ...
      >
        <div className="flex gap-8 origin-top-left">
            {(!data.version.content.sections || data.version.content.sections.length === 0) ? (
                ...
                data.version.content.sections.map((section, index) => (
                    <div key={index} className="w-max bg-white shadow-xl relative group-section">
                         {/* Selection Indicator */}
                        ...
                        >
                            {section.display_name || `${t('editor.selectionTypes.section')} ${index + 1}`}
                        </div>
                    </div>
                ))
            )}
      ...
      <div className="absolute bottom-4 left-4 bg-white rounded-lg shadow-md p-2 flex gap-2">
            <button onClick={() => setCanvasState(s => ({...s, scale: s.scale - 0.1}))} className="px-2 hover:bg-gray-100 rounded">-</button>
            <span className="text-sm w-12 text-center">{Math.round(canvasState.scale * 100)}%</span>
            <button onClick={() => setCanvasState(s => ({...s, scale: s.scale + 0.1}))} className="px-2 hover:bg-gray-100 rounded">+</button>
      </div>
    </div>
  );
