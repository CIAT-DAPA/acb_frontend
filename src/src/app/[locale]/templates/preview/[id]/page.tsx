...
export default function TemplatePreviewPage() {
  ...
  const [scrollOrientation, setScrollOrientation] = useState<
    "vertical" | "horizontal"
  >("vertical");
  const [gridColumns, setGridColumns] = useState<1 | 2 | 3 | 4 | 5 | 6>(3);
  const [gridThumbnailSize, setGridThumbnailSize] = useState<
    "small" | "medium" | "large"
  >("medium");
  ...
  useEffect(() => {
    const loadTemplate = async () => {
      ...
      try {
        ...
        const { master: templateMaster, current_version: currentVersion } =
          response.data;
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-linear-to-br from-[#f5f5dc] to-[#fff8e7] flex items-center justify-center">
  ...
  if (error || !templateData) {
    return (
      <div className="min-h-screen bg-linear-to-br from-[#f5f5dc] to-[#fff8e7] flex items-center justify-center p-4">
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-[#f5f5dc] to-[#fff8e7]">
      {/* Header */}
                ...
                <div className="flex gap-2 items-center">
                  ...
                  {[1, 2, 3, 4, 5, 6].map((num) => (
                    <button
                      key={num}
                      onClick={() =>
                        setGridColumns(num as 1 | 2 | 3 | 4 | 5 | 6)
                      }
                      className={`
                          px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all min-w-8
                          ${
                            gridColumns === num
                              ? "bg-[#606c38] text-white"
                              : "bg-white text-[#283618] border border-gray-200 hover:bg-gray-50"
